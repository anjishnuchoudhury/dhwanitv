      ...
      <div className="events py-5 px-3 px-md-0">
        <div className="container ">
          {listOfEvents.map((event, index) => {
            ...
            const checkDate = (countries.get(location) !== undefined && countries.size > 0) ? new Date(countries.get(location).date.split('-')[0], countries.get(location).date.split('-')[1] - 1, countries.get(location).date.split('-')[2]) : new Date(countries.get('US').date.split('-')[0], countries.get('US').date.split('-')[1] - 1, countries.get('US').date.split('-')[2]);
            if(checkDate !== null && (DateUtils.isFutureDay(checkDate) || DateUtils.isSameDay(new Date(), checkDate))){
              futureEvents++;
              return(
                  ...
                  <div className="d-flex align-items-center event-image event-image-overlay col-12">
                      <div className="row align-items-center">
                          ...
                          <div className="col-xl-8 col-lg-8 col-md-8 col-12 py-4">
                              ...
                              <div className="row">
                                <div className="col-12">
                                  {countries.get(location) !== undefined && countries.size > 0 ? <p className="text-white event-date-time">Date: {countries.get(location).date.split('-')[2]}<sup>{superscriptDate(countries.get(location).date.split('-')[2])}</sup>&nbsp;&nbsp;{getPrintableDate(countries.get(location).date)}.</p>: <p className="text-white event-date-time">Date: {countries.get('US').date.split('-')[2]}<sup>{superscriptDate(countries.get('US').date.split('-')[2])}</sup>&nbsp;&nbsp;{getPrintableDate(countries.get('US').date)}.</p>}
                                  <p className="text-white event-date-time">Time: {countries.get(location) !== undefined && countries.size > 0 ? twelveHourClock(countries.get(location).time) : twelveHourClock(countries.get('US').time)}.</p>
                                </div>
                              </div>
                              <div className="row pt-3">
                                <div className="col-12">
                                  {!isFree ? 
                                    isEventRegistered && isCustomerLoggedIn ? <Link to={{pathname: "/view-event", event: event}} className="btn btn-magenta col-md-6 col-12">View Event</Link> : <Link to={{ pathname: "/show-event", event: event }} className="btn btn-magenta col-md-6 col-12">Buy tickets</Link> : 
                                    <Link to={{pathname: "/view-event", event: event}} className="btn btn-magenta col-md-6 col-12">View Event</Link>
                                  }
                                </div>
                              </div>
                          </div>
