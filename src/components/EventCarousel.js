...
export default function EventCarousel({ listOfEvents, eventImages, title, display }) {
  ...
  if(listOfEvents !== null && listOfEvents !== undefined){
    return (
      <div id={`infinite-carousel-${title}`} className={`${display} justify-content-center align-items-center mb-5`}>
        <button className="control slider-button left-button position-relative pl-0"><i className="fa fa-angle-left"></i></button>
        <div className="row w-100 flex-nowrap wrapper" style={{ overflow: "hidden" }}>
          {listOfEvents.map((event, index) => {
            ...
            return (
              <div className="col-6 col-sm-5 col-md-4 col-xl-3 text-center list-item event-card" key={index}>
                {event.payment === "free" ? (
                  <a href={link} className="carousel-image-container">
                    ...
                    <div className="play-icon position-absolute">
                        <i class="fa fa-play fa-play-icon image-over-font" aria-hidden="true"></i>
        ...
        <button className="control slider-button right-button position-relative pr-0"><i className="fa fa-angle-right"></i></button>
      </div>
    )
  }else{
    return(
